...
<script type="text/javascript">
	function tf(id){
		...
		$.ajax({
			type:"POST",
			url:"http://172.16.8.15:8080/MES/tongfeng?id="+tfid,
			dataType:"jsonp",
			jsonp:"callbackparam",
						...
						<div class="x_content">
							...
							<div class="table-responsive">
								<table class="table table-striped jambo_table bulk_action">
									...
									<tbody id="tfrb">
									<f:forEach items="${requestScope.list}" var="tfrb" varStatus="status">
									<form action="tongfeng/updateTongfeng" onsubmit="return tf(${status.index})">
									<input type="hidden" name="id" value="${tfrb.id}" id="id${status.index}">
											<tr class="even pointer"> 
												<td class="a-center "><input type="checkbox"
													class="flat" name="table_records"></td>
												<td class=" ">${tfrb.id}</td>
												<td class=" ">${tfrb.didian}</td>
												<td class=" ">${tfrb.biansu}</td>
												<td class=" ">${tfrb.fengsu}</td>
												<td class=" ">${tfrb.fengliang}</td>
												<td class=" ">${tfrb.jiawan}</td>
												<td class=" ">${tfrb.eryanghuatan}</td>
												<td class=" ">${tfrb.wendu}</td>
												<td class=" ">${tfrb.shijian}</td>
												<td class="a-right a-right ">${tfrb.zhuangtai}</td>
