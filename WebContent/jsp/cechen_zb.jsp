...
<script type="text/javascript">
	$().ready(function(){
		$.ajax({
			type:"POST",
			url:"http://172.16.8.15:8080/MES/cechen",
			dataType:"jsonp",
			jsonp:"callbackparam",
			success:function(data){
				var cczb = "<tr>";
		           $.each(data,function(index,item){
		        	 cczb += "<td><input type='checkbox' class='flat' name='table_records'></td><td>"+item.id+"</td><td>"+item.didian+"</td><td>"+item.duiming+"</td><td>"+item.meiyanbie+"</td><td>"+item.yuczy+"</td><td>"+item.fangccs+"</td><td>"+item.caiyang+"</td><td>"+item.cyll+"</td><td>"+item.hxxfcnd+"</td><td>"+item.zfcnd+"</td><td>"+item.zhuangtai+"</td><td><input type='button' value='审核' class='flat' ></td></tr>"
		           })
